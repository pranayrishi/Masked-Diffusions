...
python3 -c "import json; d=json.load(open('${EVAL_FILE}')); assert d['results'], 'no eval results'; print(f'[task ${IDX}] eval ok: ' + ', '.join(f\"{r[\\\"strategy\\\"]}={r[\\\"obs_accuracy\\\"]:.4%}\" for r in d['results']))"
